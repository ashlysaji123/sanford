    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            response_data = get_response_data(
                1, redirect_url=reverse('products:product_category_list'), message="Added Successfully.")
...
def update_product_category(request, pk):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            response_data = get_response_data(
                1, redirect_url=reverse('products:product_category_list'), message="Updated")
        else:
...
def delete_product_category(request, pk):
    ...
    response_data = get_response_data(1, redirect_url=reverse(
        'products:product_category_list'), message="Deleted")
    return HttpResponse(json.dumps(response_data), content_type='application/javascript')
...
def create_product_sub_category(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            form.save()
            response_data = get_response_data(
                1, redirect_url=reverse('products:product_sub_category_list'), message="Added Successfully.")
            return HttpResponse(json.dumps(response_data), content_type='application/javascript')
        else:
            message = generate_form_errors(form)
            response_data = get_response_data(0, message=message)
...
def update_product_sub_category(request, pk):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            response_data = get_response_data(
                1, redirect_url=reverse('products:product_sub_category_list'), message="Updated")
        else:
            message = generate_form_errors(form, formset=False)
            response_data = get_response_data(0, message=message)
        return HttpResponse(json.dumps(response_data), content_type='application/javascript')
...
def delete_product_sub_category(request, pk):
    ...
    response_data = get_response_data(1, redirect_url=reverse(
        'products:product_sub_category_list'), message="Deleted")
    return HttpResponse(json.dumps(response_data), content_type='application/javascript')
...
def create_product_group(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            form.save()
            response_data = get_response_data(
                1, redirect_url=reverse('products:product_group_list'), message="Added Successfully.")
            return HttpResponse(json.dumps(response_data), content_type='application/javascript')
        else:
            message = generate_form_errors(form)
            response_data = get_response_data(0, message=message)
...
def update_product_group(request, pk):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            response_data = get_response_data(
                1, redirect_url=reverse('products:product_group_list'), message="Updated")
        else:
            message = generate_form_errors(form, formset=False)
            response_data = get_response_data(0, message=message)
        return HttpResponse(json.dumps(response_data), content_type='application/javascript')
...
def delete_product_group(request, pk):
    ...
    response_data = get_response_data(1, redirect_url=reverse(
        'products:product_group_list'), message="Deleted")
    return HttpResponse(json.dumps(response_data), content_type='application/javascript')
...
def delete_product(request, pk):
    ...
    response_data = get_response_data(1, redirect_url=reverse(
        'products:product_list'), message="Deleted")
    return HttpResponse(json.dumps(response_data), content_type='application/javascript')

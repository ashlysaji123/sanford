...
class DARTaskForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            ...
            'visit_date': DateInput(attrs={'class': 'form-control', 'placeholder': 'Date', 'id': 'Date', 'name': 'date', 'type': 'date'}),
            'available_time' :NumberInput(attrs={'class': 'required form-control', 'placeholder': 'Available time'}),
        }
    def __init__(self, user, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.fields['executive'].queryset = SalesExecutive.objects.filter(region=user.region,is_deleted=False)
        self.fields['shop'].queryset = Shop.objects.filter(country__region=user.region,is_deleted=False)

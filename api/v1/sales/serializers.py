...
from sales.models import OpeningStock, SaleItems, Sales


class OpeningStockSerializer(serializers.ModelSerializer):
    merchandiser_name = serializers.ReadOnlyField(source="merchandiser.fullname")
...
class CreateSalesSerializer(serializers.ModelSerializer):
    ...
    def update(self, instance, validated_data):
        ...
        for sale_item in sale_items_data:
            # any sale item logic here
            if SaleItems.objects.filter(
                sale=sale, product=sale_item["product"]
            ).exists():

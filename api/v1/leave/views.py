...
from .serializers import LeaveRequestSerializer,LeaveRequestListSerializer
...
def create_leave(request):
    ...
    if serializer.is_valid():
        ...
        if startdate > enddate:
            return Response({'message': "Please Check the dates!."}, status=status.HTTP_200_OK)
        date_format = "%Y-%M-%d"
...
def my_leave_reports(request):
    queryset = LeaveRequest.objects.filter(
        is_deleted=False, user=request.user).order_by('-created')

    leave_query = LeaveRequest.objects.filter(is_deleted=False, user=request.user,is_approved=True)
    total_leave_taken = 0

...
from .serializers import MarkAttendanceSerializer,AttendanceSerializer
...
def MarkAttendanceOut(request,pk):
    ...
    * Method: POST Only
    ...
    serializer = MarkAttendanceSerializer(attandance,data=request.data,context={"request": request},partial=True)
    if serializer.is_valid():
...
def MyAttandanceSummaryView(request):
    ...
    queryset = Attendance.objects.filter(
        ...
        check_in_time__range=[start_week, end_week],is_leave=False
        )
    ...
    if query == 'monthly':
        ...
        queryset = Attendance.objects.filter(
            user=request.user,
            is_deleted=False,check_in_time__month=month,is_leave=False
            )
    if query == 'alltime':
        queryset = Attendance.objects.filter(user=request.user,is_deleted=False,is_leave=False)
    # Daily query sum
